...
function loadConfig(array $options = [])
{
    ...
    if (!empty($config['ADLISTS_URL'])) {
        printAndLog('Configuration variable ADLISTS_URL is deprecated, use BLOCKLISTS_URL instead!' . PHP_EOL, 'WARNING');
    }
    ...
    foreach ($domainListTypes as $typeName => $typeId) {
        ...
        if (!empty($configSectionData[$url_key])) {
            ...
            if ($contents !== false) {
                ...
                $typeId > 0 && $sth->bindParam(':type', $typeId, PDO::PARAM_INT);
                ...
                if ($configSectionData['GROUP_EXCLUSIVE'] === true) {
                    $sql = 'SELECT * FROM `adlist` LEFT JOIN `adlist_by_group` ON `adlist`.`id` = `adlist_by_group`.`adlist_id` WHERE `adlist`.`enabled` = 1 AND `adlist_by_group`.`group_id` = :group_id';

                    $typeId > 0 && $sql .= ' AND `adlist`.`type` = :type';
